...
fn transfer_particles_to_grid(
    ...
    for i in 0..grid.cell_mass.len() {
        if grid.cell_mass[i] > 0.0 {
            grid.cell_velocity[i] =
                grid.cell_momentum[i] / grid.cell_mass[i] + params.gravity * params.dt;
        }
    }
}
...
fn render_grid(mut gizmos: Gizmos, grid: Res<Grid>, params: Res<SimParams>) {
    ...
    for y in 0..=grid.dimensions.y {
        ...
        gizmos.line_2d(
            ...
            Vec2::new(
                grid.origin.x + grid.dimensions.x as f32 * grid.cell_size,
                y_pos,
            ),
            color,
        );
    ...
    for x in 0..=grid.dimensions.x {
        ...
        gizmos.line_2d(
            ...
            Vec2::new(
                x_pos,
                grid.origin.y + grid.dimensions.y as f32 * grid.cell_size,
            ),
            color,
        );
    }
